...
Storage: Pinecone Quantum Drive
`;
        break;
  ...
  return (
    <div 
      className={`
        text-ed-amber-400 font-mono
        ${isFullscreen 
          ? 'fixed inset-0 z-50 bg-ed-bg-primary/95 backdrop-blur-xl' 
          : isMobile 
            ? 'w-full min-h-0 flex flex-col mobile-terminal-container' 
            : 'bg-ed-bg-primary/95 backdrop-blur-xl w-full max-w-4xl rounded-3xl border border-ed-green-500/30'
        }
      `}
    >
      {/* Terminal Header */}
      <div className={`
        flex items-center justify-between px-4 py-2 bg-black/50 border-b border-ed-green-500/30
        ${isMobile ? 'min-h-[60px]' : ''}
      `}>
      ...
      {/* Terminal Footer */}
      <div className={`
        px-4 py-2 border-t border-ed-green-500/30 bg-black/50 text-xs text-gray-400
