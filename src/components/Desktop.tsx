...
// Removed: loadingGif - replaced with pure CSS CRT effects
import { 
  Settings, 
...
const Desktop: React.FC = () => {
  ...
  // Initialize particle system - Electric Dreams colors
  useEffect(() => {
    const colors = ['#00FFFF', '#FFB000', '#FF00FF', '#00B8B8'];
      ...
      `}>
        <div className={`space-y-2 ${isMobile ? 'mobile-metric-item' : ''}`}>
          <div className="flex items-center justify-between">
                            <span className="text-ed-green-500/80">CPU</span>
            <span className={`text-white ${isMobile ? 'mobile-metric-value' : ''}`}>
              {systemMetrics.cpu}%
            </span>
          </div>
                      <div className="h-1 bg-ed-green-500/20 rounded-full">
              <div className="h-1 bg-ed-green-500 rounded-full" style={{ width: `${systemMetrics.cpu}%` }} />
          </div>
        </div>
        <div className={`space-y-2 ${isMobile ? 'mobile-metric-item' : ''}`}>
          <div className="flex items-center justify-between">
            <span className="text-ed-green-400/80">Memory</span>
            <span className={`text-white ${isMobile ? 'mobile-metric-value' : ''}`}>
              {systemMetrics.memory}%
            </span>
          </div>
                      <div className="h-1 bg-ed-green-500/20 rounded-full">
              <div className="h-1 bg-ed-green-500 rounded-full" style={{ width: `${systemMetrics.memory}%` }} />
          </div>
        </div>
        <div className={`space-y-2 ${isMobile ? 'mobile-metric-item' : ''}`}>
          <div className="flex items-center justify-between">
            <span className="text-ed-green-400/80">Disk</span>
            <span className={`text-white ${isMobile ? 'mobile-metric-value' : ''}`}>
              {systemMetrics.diskUsage}%
            </span>
          </div>
                      <div className="h-1 bg-ed-green-500/20 rounded-full">
              <div className="h-1 bg-ed-green-500 rounded-full" style={{ width: `${systemMetrics.diskUsage}%` }} />
          </div>
        </div>
        <div className={`space-y-2 ${isMobile ? 'mobile-metric-item' : ''}`}>
          <div className="flex items-center justify-between">
            <span className={`text-ed-green-400/80 ${isMobile ? 'mobile-metric-label' : ''}`}>
              Uptime
            </span>
  ...
  const folders: FolderItem[] = [
    {
      ...
      color: 'from-ed-green-800 to-ed-bg-primary',
    ...
    },
    {
      id: 'docs',
        ...
        color: 'from-ed-amber-700 to-ed-bg-secondary',
  ...
  return (
    <div className="min-h-screen w-full relative overflow-hidden">
      {/* Notification Bar */}
      <div className={`
        fixed top-0 left-0 right-0 z-50 bg-black/90 backdrop-blur-xl border-b border-ed-green-700/20
            ...
            {!isMobile && (
              <div className="flex items-center space-x-3">
                <a 
                  ...
                  className="p-2 rounded-lg bg-ed-green-800/10 hover:bg-ed-green-800/20 cursor-pointer transition-all duration-300 group border border-ed-green-700/20"
                  title="OmniaOS X"
                >
          ...
          <div className="flex items-center space-x-6">
            ...
            {isMobile ? (
              <div className="flex items-center space-x-2">
                <div className="flex items-center space-x-1 text-ed-green-300/80">
                  <Cpu size={12} />
              ...
              <div className="flex items-center space-x-6">
                <div className="flex items-center space-x-2 text-ed-green-300/80">
                  <Cpu size={16} className="text-ed-green-400/60" />
                  <span className="text-sm font-light tracking-wide">{systemMetrics.cpu}%</span>
                </div>
                <div className="flex items-center space-x-2 text-ed-green-300/80">
                  <MemoryStick size={16} className="text-ed-green-400/60" />
                  <span className="text-sm font-light tracking-wide">{systemMetrics.memory}%</span>
                </div>
                <div className="flex items-center space-x-2 text-ed-green-300/80">
                  <Network size={16} className="text-ed-green-400/60" />
                  <span className="text-sm font-light tracking-wide">{systemMetrics.network}%</span>
                </div>
                <div className="flex items-center space-x-2 text-ed-green-300/80">
                  <Thermometer size={16} className="text-ed-green-400/60" />
            ...
            {!isMobile && (
              <div className="flex items-center space-x-3">
                <div className="p-1.5 rounded-lg bg-ed-green-700/10 hover:bg-ed-green-700/20 cursor-pointer border border-ed-green-500/20 transition-all duration-300">
                  <Wifi size={16} className="text-ed-green-300" />
                </div>
                <div className="p-1.5 rounded-lg bg-ed-green-700/10 hover:bg-ed-green-700/20 cursor-pointer border border-ed-green-500/20 transition-all duration-300">
                  <Volume2 size={16} className="text-ed-green-300" />
                </div>
                <div className="p-1.5 rounded-lg bg-ed-green-700/10 hover:bg-ed-green-700/20 cursor-pointer border border-ed-green-500/20 transition-all duration-300">
                  <Battery size={16} className="text-ed-green-300" />
                </div>
              </div>
            )}
            ...
            <div className={`
              flex items-center space-x-2 text-ed-text-primary/80 font-light
            ...
            `}>
              {isMobile ? (
                ...
                <>
                  <Calendar size={16} className="text-ed-green-400/60" />
                  <span className="text-sm tracking-wide">{currentTime.toLocaleDateString()}</span>
                  <Clock size={16} className="text-ed-green-400/60" />
      ...
      {/* Background Gradients - Electric Dreams Sentient Grid */}
      <div className="fixed inset-0 z-0">
        {/* Base gradient - Deep black with subtle green tint */}
        <div
        ...
        {/* 80s PERSPECTIVE GRID FLOOR - The Sentient Grid */}
        <div
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              animation: 'sentient-grid-scroll 15s linear infinite',
          ...
          style={{
            background: `
              repeating-linear-gradient(
                ...
                transparent,
                transparent 200px,
                rgba(255,0,255,0.05) 200px,
                rgba(255,0,255,0.05) 202px
              ),
              repeating-linear-gradient(
                -45deg,
                transparent,
      ...
      {/* CRT Screen Vignette & Curvature Effect */}
      <div
      ...
      {/* ══════════════════════════════════════════════════════
          SYSTEM DIAGNOSTICS - DOS/ASCII Style Monitor
          ══════════════════════════════════════════════════════ */}
      {!isMobile && (
        <div className="fixed bottom-24 right-8 z-10">
          ...
          >
            ...
            {/* Metrics with ASCII progress bars */}
            <div className="space-y-2">
              {[
                { label: 'CPU', value: systemMetrics.cpu },
                { label: 'MEM', value: systemMetrics.memory },
                { label: 'NET', value: systemMetrics.network },
                { label: 'TMP', value: systemMetrics.temperature }
              ].map(({ label, value }) => {
      ...
      `}>
          ...
          `}>
            {folders.map((folder, i) => (
              <div
                ...
                style={{
                  animation: `folder-boot 0.6s ease-out ${i * 100}ms both`
                }}
              >
                {/* CRT-Style Icon Container */}
                <div
        ...
        {isMobile ? (
          ...
          <div className="mobile-widget-grid">
            {widgets.filter(widget => widget.name !== 'System Monitor').map(widget => (
              <div
                key={widget.id}
                className="mobile-widget bg-black/80 backdrop-blur-xl border border-ed-green-700/30 rounded-2xl overflow-hidden"
              >
                {/* Widget Header */}
                <div className="flex items-center justify-between p-3 bg-ed-green-700/10 border-b border-ed-green-700/20">
          ...
          widgets.map(widget => (
            <div
              key={widget.id}
              className="fixed z-30 bg-black/80 backdrop-blur-xl border border-ed-green-700/30 rounded-2xl overflow-hidden"
              style={{
                left: widget.x,
                top: widget.y,
                width: widget.width,
        ...
        {showCommandPalette && (
          ...
          `}>
            <div className={`
              bg-gray-900/95 backdrop-blur-xl rounded-2xl border border-ed-green-500/30 overflow-hidden
              ${isMobile 
            ...
            `}>
              {/* Search Header */}
              <div className={`
                flex items-center border-b border-ed-green-500/20
                ${isMobile ? 'p-4 pt-6' : 'p-4'}
              `}>
                <Command className={`text-ed-green-400 mr-3 ${isMobile ? 'w-6 h-6' : 'w-5 h-5'}`} />
                <input
                  type="text"
                  ...
                  <div className={isMobile ? 'p-2' : 'p-2'}>
                    {filteredActions.map(action => (
                      <button
                        ...
                        className={`
                          command-item w-full flex items-center gap-3 rounded-xl hover:bg-ed-green-500/10 transition-colors text-left
                          ${isMobile ? 'p-4' : 'p-3'}
                        `}
                      >
                        <div className="text-ed-green-400">{action.icon}</div>
        ...
        {showContextMenu && (
          <div
            className={`
              fixed z-50 bg-gray-900/95 backdrop-blur-xl border border-ed-green-500/30 py-2
              ${isMobile 
          ...
          >
            <button
              ...
              className={`
                context-item w-full text-left hover:bg-ed-green-500/10 flex items-center gap-3 text-white
                ${isMobile ? 'px-6 py-4' : 'px-4 py-2'}
              `}
            >
              <Plus size={isMobile ? 20 : 16} className="text-ed-green-400" />
              Add Widget
            </button>
            <button
              onClick={() => {
              ...
              `}
            >
              <Command size={isMobile ? 20 : 16} className="text-ed-green-400" />
              Command Palette
            </button>
            <hr className="my-2 border-ed-green-500/20" />
            <button
              onClick={() => {
              ...
              `}
            >
              <Settings size={isMobile ? 20 : 16} className="text-ed-green-400" />
              Settings
            </button>
            <button
              onClick={() => {
              ...
              `}
            >
              <RotateCw size={isMobile ? 20 : 16} className="text-ed-green-400" />
              Refresh
            </button>
          </div>
        )}
        ...
        {showQuickLaunch && (
          ...
          `}>
            <div className="bg-black/90 backdrop-blur-xl rounded-2xl border border-ed-green-500/30 p-4">
              <div className={`
                flex items-center
                ${isMobile ? 'flex-col space-y-4' : 'space-x-4'}
              `}>
                <div className="text-ed-green-400 text-sm font-medium">Quick Launch</div>
                <div className={`
                  flex items-center
                ...
                `}>
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`
                        group relative rounded-xl bg-ed-green-500/10 hover:bg-ed-green-500/20 text-ed-green-400 hover:text-ed-green-300 transition-all duration-300
                        ${isMobile 
        ...
        {/* Keyboard Shortcuts Help - Terminal Style */}
        {!isMobile && (
          <div
            className="fixed bottom-24 right-[360px] z-10 font-mono text-[10px]"
            style={{
          ...
          >
            <div className="text-ed-green-500/80">┌─ HOTKEYS ─────────────┐</div>
            <div className="text-ed-green-400/70 mt-1">│ ⌘K/Ctrl+K  CMD_PALETTE │</div>
            <div className="text-ed-green-400/70">│ Ctrl+Space QUICK_LAUNCH│</div>
            <div className="text-ed-green-400/70">│ Right-Click CONTEXT    │</div>
            <div className="text-ed-green-500/80">└────────────────────────┘</div>
          </div>
        )}
      ...
      <style>{`
        ...
        /* Sentient Grid - 80s perspective grid scrolling towards viewer */
        @keyframes sentient-grid-scroll {
          from { background-position: 0 0; }
        ...
        /* Glowing text pulse for terminal */
        ...
        /* Retro folder hover effect */
        @keyframes folder-boot {
          0% {
            transform: scale(0.8);
            opacity: 0;
